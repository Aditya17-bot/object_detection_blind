...
def build():
    ...
    doc.body(
        ...
        "the correct warning. These are small numbers reviewed by one of us, "
        "so we report them as a sanity check on the deterministic core and not "
        "as a perception result.",
        first=True)
    doc.body(
        "The system’s answer to those wrong names was a confidence gate: "
        "speak the generic word “obstacle” when detection confidence falls "
        "below a threshold. We report it as a negative result, because it does "
        "not work and because the reason is the sharpest evidence we have for "
        "C1. The threshold came from a probe placing misnames at 0.65–0.75 and "
        "correct names at 0.85 and above. Re-measured on the same clips, the "
        "dustbin is labelled “toilet” at a peak confidence of 0.94 and the "
        "wardrobe “refrigerator” at 0.82, against 0.92 for a correctly named "
        "chair; the distributions overlap and no threshold separates them. The "
        "failure is structural rather than a matter of calibration. Detection "
        "confidence reports how sure the detector is that a box holds one of "
        "its eighty classes, and that vocabulary contains no word for a "
        "wardrobe, so a forced choice among the available words can be made "
        "with complete certainty and still be wrong. This is exactly the "
        "failure C1 predicts: an abstention criterion borrowed from a "
        "neighbouring layer instead of derived from the failure mode of the "
        "layer it guards. Its replacement moves the naming decision onto the "
        "distance between an embedding of the detected crop and a small set of "
        "user-labelled examples, abstaining when the nearest example is not "
        "clearly closer than the next competing label. The user labelled 280 "
        "crops harvested from their own clips into 17 classes in a single "
        "30-minute pass; the detector's own backbone supplies the embedding, "
        "so there is no second model and no training run.")
    doc.body(
        "That criterion is separable where confidence was not. A "
        "leave-one-out sweep finds an operating point naming 49 of 280 crops "
        "with 49/49 correct and zero wrong names, against 10 errors at the "
        "untuned default. Two properties of the sweep matter more than the "
        "headline. The discriminating quantity is the margin over the "
        "runner-up label, not the absolute distance: the similarity floor is "
        "inert anywhere in 0.50-0.65, so what separates a correct rename from "
        "a wrong one is how much better the winning label is than its nearest "
        "competitor, not how good it is alone. And the selected point is "
        "deliberately low-coverage, declining to name 82% of crops, which is "
        "the intended shape for a mechanism whose output is spoken as fact.")
    doc.body(
        "On the recorded clips the mechanism performs 105 renames across "
        "eight distinct patterns; we inspected every pattern and 104 are "
        "correct. Five of the eight are errors our own earlier evaluation had "
        "recorded as unfixable consequences of COCO's label set, including the "
        "dustbin and the wardrobe. One is of a different kind: on four frames "
        "the detector reports a person where a blanket is draped over a chair, "
        "and the naming head corrects it to chair - a hallucinated pedestrian "
        "is a materially worse error for a blind traveller than a mislabelled "
        "bin, and it is removed without touching the detector. The single "
        "arguable rename is a box containing a bench with a suitcase on it, "
        "named suitcase; both are obstacle classes, so the warning is "
        "unchanged in kind. On the two clips containing none of the labelled "
        "objects the mechanism makes no renames at all. The labelled examples "
        "and the clips come from the same rooms and the renames were "
        "adjudicated by the author; a calibrated evaluation on held-out rooms "
        "remains future work.")
    doc.body(
        "One methodological point generalises beyond this component. Our "
        "first leave-one-out report scored a predicted do-not-name label as a "
        "wrong name, when at runtime that prediction is an abstention, and "
        "consequently reported that no zero-error operating point existed - "
        "the exact opposite of the truth. An evaluation harness for an "
        "abstaining component must mirror the runtime decision function "
        "exactly; if it misprices abstention it selects the wrong operating "
        "point and does so silently. We now assert that equality in a test.")
    doc.body(
